...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W'+str(hidd_len+1)] = np.random.randn(shape_one, num_classes) * weight_scale 
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          ...
          if j == self.num_layers:
            ...
            if self.use_dropout and self.use_batchnorm:
              ...
              output[j], cache[i] = affine_batchnorm_relu_dropout_forward(output[i], W_j, b_j, gamma_j, beta_j, self.bn_params[i], self.dropout_param)
              ...
              gamma_j = self.params[gammas]
              beta_j = self.params[betas]
              output[j], cache[i] = affine_batchnorm_relu_forward(output[i], W_j, b_j, gamma_j, beta_j, self.bn_params[i])
        ...
        dx, last_weight, last_bias = affine_backward(dout, cache[self.num_layers-1])
        grads['W'+str(self.num_layers)] = last_weight + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = last_bias
        # weights, bias, gamma, beta of the rest layers
        if self.use_dropout and self.use_batchnorm:
          for i in range(hidd_len, 0, -1):
                dx, dw, db, dgamma, dbeta = affine_batchnorm_relu_dropout_backward(dx, cache[i-1])
          ...
          for i in range(hidd_len, 0, -1):
                dx, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dx, cache[i-1])
                grads['W'+str(i)] = dw + self.reg * self.params['W'+str(i)]
                grads['b'+str(i)] = db
                grads['gamma'+str(i)] = dgamma
                grads['beta'+str(i)] = dbeta
