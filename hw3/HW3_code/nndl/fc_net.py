...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    doutput[hidden_dims], grads[weight_last], grads[bias_last] = affine_backward(dx, cache[hidden_dims])
    grads[weight_last] = grads[weight_last] + self.reg * self.params[weight_last]

    for i in range(hidden_dims):
      weight = str('W') + str(hidden_dims - i)
      bias = str('b') + str(hidden_dims - i)
      doutput[hidden_dims - i -1], grads[weight], grads[bias] = affine_relu_backward(doutput[hidden_dims - i], cache[hidden_dims - i -1])
